...
amp_sem_t request_queue_sem;amp_sem_t begin_send;
...
void* send_work(void* arg) {
	...
	while (1) {
		...
		sprintf(msgp->msg, "this is No.%d server:%d", SERVER_ID1, i);
...
//		printf("send message %d begin\n", i);
        printf("remote_type: %d, remote_id: %d .....\n",  req->req_remote_type, req->req_remote_id);
		err = amp_send_sync(this_ctxt, req, CLIENT, CLIENT_ID2, 1);	//5、发送消息
		if (err < 0) {
			printf("send error, err:%d\n", err);
			continue;
		}
//		printf("send message %d end\n", i);

		msgp =
				(test_msg_t *) ((char *) req->req_reply + AMP_MESSAGE_HEADER_LEN);

		printf("SERVER_SEND_WORKER: send message %ld-%d\n",  pthread_self(),i);
...
//			int i = 0;
//			amp_kiov_t *tmp_iov = NULL;
//			tmp_iov = req->req_iov;
//			if (!tmp_iov) {
//				printf("request tmp kiov empty!\n");
//				return NULL;
//			}
//			for (i = 0; i < req->req_niov; i++) {
//				printf("%dst:\n%s\n", i + 1, tmp_iov[i].ak_addr);
//			}
//		}

		//设置返回消息
		sprintf(msgp->msg, "Server: we reply to client:%d, seqno:%d\n",
				req->req_remote_id, msgp->seqno);
		printf("++++++++++++++++++SERVER_LISTEN_WORK: receivd message  %ld-%d\n",  pthread_self(),msgp->seqno);
...
int main(int argc, char *argv[]) {
	...
	this_ctxt = amp_sys_init(SERVER, SERVER_ID1); //1、初始化系统
	if (!this_ctxt) {
...
#if 0
	//创建进程
	amp_sem_init_locked(&begin_send);
