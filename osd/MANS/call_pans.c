...
uint8_t *compress_pans(uint8_t * input_data, size_t input_size, size_t * output_size );
...
int main(int argc, char* argv[])
{
	...
	if(fdw <= 0){
	 ...
	 src_buf = (uint8_t *)malloc(comp_length);
	...
	if(src_buf == NULL){
	 ...
	 while(src_len > 0){
		 if(src_len > 0){
			...
			dest_buf = compress_pans(src_buf, src_len, &dest_len);
