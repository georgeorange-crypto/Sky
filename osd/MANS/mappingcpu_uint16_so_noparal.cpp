...
int compress(
    ...
    //std::vector<int>& i_output_lengths,
    //std::vector<uint16_t>& i_centers,
    //std::vector<uint8_t>& i_codes,
    //std::vector<uint8_t>& i_bit_signals
    ...
    uint16_t * centers = NULL;
    uint8_t  * codes = NULL;
    uint8_t  * bit_signals = NULL;
    size_t      buf_offset = 0;
    char *  output_buf = (char *)output_data;
    size_t  * metas; 



    size_t  bit_signal_size = 0;
    ...
    metas = (size_t*) output_buf;
    buf_offset += 5*sizeof(size_t);
    output_lengths = (int *)(&(output_buf[buf_offset]));
    
    buf_offset += (gsize+1)*sizeof(int);
    centers = (uint16_t *)(&(output_buf[buf_offset]));
    
    buf_offset += gsize*sizeof(uint16_t);
    codes = (uint8_t *)(&(output_buf[buf_offset]));
    
    buf_offset += num_elements*sizeof(uint8_t);
    bit_signals = (uint8_t *)(&(output_buf[buf_offset]));

    metas[0]=(size_t) ((gsize+1)*sizeof(int));
    metas[1]=(size_t) ((gsize)*sizeof(uint16_t));
    metas[2]=(size_t) (num_elements*sizeof(uint8_t));
    ...
    if(total_bit_bytes > num_elements*sizeof(uint16_t))
	    return -ENOMEM;
    //std::memset((uint8_t *)bit_signals,0, total_bit_bytes);
    bit_signal_size = (size_t)total_bit_bytes;
    metas[3]= bit_signal_size; 
    ...
    for (int thread_idx = 0; thread_idx < total_threads; ++thread_idx) {
        ...
        //const uint8_t* src = tmp_bit_signals.data();
        ...
        //printf("compile signal passed\n");
    // 设置 元数据
    metas[4] = metas[0]+metas[1]+metas[2]+metas[3];
...
void  decompress(
    ...
    size_t dmetas[],
    char * src,
    //int * output_lengths,             // gsize
    //uint16_t* centers,               // gsize
    //uint8_t * codes,                  // num_elements
    //uint8_t *  bit_signals,            // bitstream
...
)
{
    ...
    size_t start_offset = 5*sizeof(size_t);
    
    data_ptr = &(src[start_offset]);
    output_lengths = (int*)data_ptr;

    start_offset += dmetas[0];
    data_ptr = &(src[start_offset]);
    centers = (uint16_t *)data_ptr;
    
    start_offset += dmetas[1];
    data_ptr = &(src[start_offset]);
    codes = (uint8_t *)data_ptr;

    start_offset += dmetas[2];
    data_ptr = &(src[start_offset]);
    bit_signals = (uint8_t *)data_ptr;



    //int num_elements = codes.size();
    int num_elements = (int)(dmetas[2]/sizeof(uint8_t));
    //int gsize = output_lengths.size();
    int gsize = (int)(dmetas[0]/sizeof(int));
    int total_threads = gsize * cmp_tblock_size;
...
char * adm_compress_data(char * src, size_t src_len , size_t* pdest_len)
{
   ...
   // 全局参数
    ...
    size_t total_size = 5*sizeof(size_t) + sizeof(int)*(gsize+1) + sizeof(uint16_t)*gsize + sizeof(uint8_t)*num_elements + sizeof(uint8_t)*src_len;  
    char * merged = (char *)calloc(total_size,1);
    if(merged == NULL){
    ...
    {
        auto start = std::chrono::high_resolution_clock::now();
        int ret = compress((uint16_t*)src, (char*)merged, num_elements);
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> duration = (end - start);
        if(duration.count() < exe_time) exe_time = duration.count();
	if(ret != 0){
		printf("compreess_failed error code %d\n", ret);
		free(merged);
		return NULL;
	}
    }
    ...
    size_t len1 = gsize * sizeof(int);
    size_t len2 = (gsize+1) * sizeof(uint16_t);
    size_t len3 = num_elements * sizeof(uint8_t);
    size_t len4 = ((size_t)output_lengths.data()[1]);;
    len4 <<= 32;
    len4 = len4+(size_t)output_lengths.data()[1];
    ...
    total_size = 5*sizeof(size_t)+metas[4];
...
char * adm_decompress_data(char * src, size_t ori_len , size_t* pdest_len)
{
...
#endif
	// 分配decompress_buf
	ori_data = (char*)malloc(ori_len*5/2);
	if(ori_data == NULL){
		printf("can not allocate buf for decompress , len %ld \n", ori_len);
		*pdest_len = 0;
	}
	out_data = (uint16_t *) ori_data;
	...
	// 执行 decompress
        ...
        //auto start = std::chrono::high_resolution_clock::now();
        	...
        	decompress(d_metas, src, out_data, &recover_data_len);
        //auto end = std::chrono::high_resolution_clock::now();    // 计时结束
        //std::chrono::duration<double, std::milli> duration = (end - start);
        //if(duration.count() < exe_time) exe_time = duration.count();
    	}
	//printf("decompress return %lu\n", recover_data_len);
	*pdest_len = recover_data_len;
	if(recover_data_len <= 0){
		free(ori_data);
		ori_data = NULL;
	}
