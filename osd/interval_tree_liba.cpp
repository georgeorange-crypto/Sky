...
struct IntervalTree* get_IntervalTree_handle()
{
	...
	T->root = new IntervalTNode();
	cp_tnode(T->root, NIL);
	pthread_rwlock_init(&T->Tree_rwlock, NULL);

	struct IntervalTree* T_ret = (struct IntervalTree*) T;
...
struct interval * do_search_exact_interval(struct IntervalTree* T, struct interval inte){

	fprintf(stderr, "do_search exact in Tree. %p \n", T);
...
void do_IntervalT_Delete(struct IntervalTree* T, IntervalTNode* node)
{ 
