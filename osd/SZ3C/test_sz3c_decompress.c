...
int main(int argc, char * argv[])
{
	...
	r2 = r3 = r4 = r5 = 0;
	if(argc < 6){
		printf("test_sz3c_compress srcfile destfile recsize dataType  r1[r2 r3 r4 r5] \n");
		return 0;
	}
	...
	//printf("try to alloc decompress data , size %lu\n", rec_size);
	src_data = (char *)malloc(rec_size);

	//printf("alloced decompress data , ptr %p \n", src_data);
	if(src_data == NULL){
	...
	// TODO: decompress changeed here 

	ret = read(fd_src, &cmeta, sizeof(cmeta));
	ret = read(fd_src, src_data, cmeta.compressed_size);
	//cmeta.ori_offset = 0;
	...
	while(ret >= 0){
		...
		if (errBoundMode == PSNR)
			pwrBoundRatio = errBoundRatio;
               */
		compressed_size = cmeta.compressed_size;
		dest_data = SZ_decompress(dataType, src_data, compressed_size,
		...
		//total_size += ret_w;
		ret_w = write(fd_dest, dest_data, rec_size);
		if(ret_w <= 0)
			break;
		total_size += ret_w;
		ori_size += compressed_size;
		cmeta.ori_offset = ori_size;
		free(dest_data);
		dest_data = NULL;
		ret = read(fd_src, &cmeta, sizeof(cmeta));
		if(ret <= 0){
			printf("try to read reccord size %lu, got %lu, return \n", sizeof(cmeta), ret);
			break;
		}
		unsigned int next_compressed_size = (unsigned int)cmeta.compressed_size;
		//printf("next compresed size %d\n", next_compressed_size);

		ret = read(fd_src, src_data, (int)(cmeta.compressed_size));
		//printf("try to get compressed data len %lu, got %lu \n", cmeta.compressed_size, ret);


	}
	
	printf("compressed_size %lu, recover_size %lu, compress Ratio %f \n", ori_size, total_size, ((float)total_size)/((float)ori_size));
	if(src_data)
		free(src_data);
	close(fd_src);
	close(fd_dest);
