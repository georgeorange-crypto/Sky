                ...
                skyfs_u32_t *conflict_index)
{
    ...
    if (rc >= 0) {
        ...
        *conflict_index = msgp->u.createAck.meta.conflict_index;
        SKYFS_MSG("__skyfs_C2M_create:%s,ino:%llu,conflict:%u\n", name, *ino, *conflict_index);
    }else{
                ...
                struct stat *sbbuf)
{
    ...
    SKYFS_ENTER("__skyfs_C2M_getattr:ino:%llu,conflict:%u\n", ino, conflict_index);
    ...
    rc = amp_send_sync(client_comp_context, req, SKYFS_MDS, mds_id, 1);
    if(rc < 0){
        SKYFS_ERROR("__skyfs_C2M_getattr:send failed.rc:%d\n",rc);
           goto err_out;
    }
    ...
    if (rc >= 0) {
        ...
        sbbuf->st_blocks = (skyfs_u32_t)(meta->size) >> 9;
        /*
                ...
                skyfs_m_setmeta_args_t *args)
{
    ...
    SKYFS_ENTER("__skyfs_C2M_setattr:enter:ino:%llu,conflict:%u\n", 
        ino, conflict_index);
    ...
    rc = amp_send_sync(client_comp_context, req, SKYFS_MDS, mds_id, 1);
    if(rc < 0){
        SKYFS_ERROR("__skyfs_C2M_setattr:send failed.rc:%d\n",rc);
           goto err_out;
    }
...
err_none:
    ...
    SKYFS_LEAVE("__skyfs_C2M_setattr:exit:rc:%d\n", rc);
    ...
    if (rc > 0) {
        ...
        *conflict_index = msgp->u.lookupAck.meta.conflict_index;
		...
		if(rc0 == 0){
			...
			__skyfs_C_update_meta(*ino, *conflict_index, &(msgp->u.lookupAck.meta));
		}
    }
        ...
        skyfs_u32_t count)
{
    ...
    if(offset == 0)
    	SKYFS_ERROR("__skyfs_C2M_readdir:enter,ino:%llu,offset:%llu,count:%u\n", 
        	ino, offset, count);
	...
	//__skyfs_C_release_bcache(ino);
	    ...
	    bcache = __skyfs_C_getbcache(subset_id, chunk_id, 
                    ino, offset, client_this_id, pid);
        }else if(bcache->now_index == 0){
            SKYFS_ERROR("__skyfs_C2M_readdir:reach end., offset %lu try next , cur_bcache %p \n", offset, bcache);
	    ...
	    /*add by mayl end */
	    if(bcache == NULL){
		SKYFS_ERROR("GET next bcache NULL \n");
            	bzero(dentry, sizeof(skyfs_dentry_t)); 
            	dentry->offset = 0;
	...
	if(bcache == NULL){
        SKYFS_ERROR("__skyfs_C2M_readdir:bcache is NULL. offset, %lu, offset\n");
	bzero(dentry, sizeof(skyfs_dentry_t)); 
        dentry->offset = 0;
        memcpy(buf_page, dentry, sizeof(skyfs_dentry_t)); 
		goto EXIT;
    ...
    if(buf_off == 0 ){
	    ...
	    chunk_id = bcache->chunk_id;	
	    subset_id = bcache->subset_id;	
	    SKYFS_ERROR("C2M_readdir small bcache ,get next, buf_off %d, off %d,  bcache %p subset_id %d, chunk_id %d \n",
